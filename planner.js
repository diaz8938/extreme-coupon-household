(() => {
  ...
  function bindPlanner(){
    ...
    $p('#addCustomPlanItem').onclick = () => {
      ...
      if(!name) return;
      const list = planState.list; list.push({product_id:'custom_'+Date.now(),name,qty,custom:true}); planState.list = list;
      $p('#customPlanName').value = ''; renderShoppingList();
    };
    $p('#quickAddUsual').onclick = () => {
      const list = planState.list;
      catalog.forEach(c => {if(!list.find(x => x.product_id === c.product_id)) list.push({product_id:c.product_id,name:c.name,size:c.size,qty:Math.max(1,c.usual_qty||1),custom:false});});
      planState.list = list; renderShoppingList();
    };
  ...
  function renderShoppingList(){
    ...
    el.innerHTML=list.map((x,i)=>{const c=catalog.find(c=>c.product_id===x.product_id);const verifiedCount=c?Object.values(c.stores||{}).filter(v=>v.verified&&v.price!=null&&!(v.requires_unit_normalization&&!v.unit_normalized)).length:0;const badge=x.custom?'<span class="pill pWarn">needs price</span>':`<span class="pill ${verifiedCount>1?'pGood':'pWarn'}">${verifiedCount} store${verifiedCount===1?'':'s'} verified</span>`;return `<div class="planItem"><div><b>${esc(x.name)}</b><small>${esc(x.size||'')}${x.custom?'custom item':''}</small>${badge}</div><input type="number" min="1" step="1" value="${x.qty}" data-plan-qty="${i}" aria-label="Quantity"><button class="btn danger removeMini" data-plan-remove="${i}">×</button></div>`;}).join('');
    $$p('[data-plan-qty]').forEach(inp=>inp.onchange=e=>{const list=planState.list;list[+e.target.dataset.planQty].qty=Math.max(1,Math.floor(Number(e.target.value)||1));planState.list=list;renderShoppingList();});
  ...
  function lineForOption(item,opt){const base=opt.unitPrice*item.qty,coupon=bestLinkedCoupon(item.product_id,opt.store,item.qty),couponValue=Math.min(base,Number(coupon?.value)||0);return{...opt,item,base,coupon,couponValue,effective:base-couponValue};}
