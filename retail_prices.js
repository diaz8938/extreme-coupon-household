(() => {
  ...
  async function fetchAuto(code,identity){
    const cache=get('ec_autoRetailPriceCache',{}),key=identityKey(code,identity),c=cache[key];
  ...
  async function attach(code,identity){
    ...
    try{const data=await fetchAuto(code,identity);if(activeCode!==code||identityKey(code,activeIdentity)!==signature)return;render(code,identity,data);}catch(e){if(activeCode!==code||identityKey(code,activeIdentity)!==signature)return;render(code,identity,null,e?.name==='AbortError'?'Price lookup timed out. Double-check links still work.':`Auto price lookup unavailable: ${e?.message||e}`);}
  }
