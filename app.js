...
const state = {
  ...
  get priceObservations(){ return JSON.parse(localStorage.getItem('ec_priceObservations') || '[]'); },
  set priceObservations(v){ localStorage.setItem('ec_priceObservations', JSON.stringify(v)); }
};
...
function renderHome(){
  $('#summary').innerHTML = `<div class="k"><b>${hebReceipt.items_purchased}</b><span>items</span></div><div class="k"><b>${money(hebReceipt.total_sale)}</b><span>paid</span></div><div class="k"><b>${money(hebReceipt.total_savings)}</b><span>receipt savings</span></div>`;
  const alerts = comparisonData.comparisons.map(c=>scoreComparison(c)).filter(x=>x && x.impact >= household.optimization_rules.minimum_item_switch_savings).sort((a,b)=>b.impact-a.impact).slice(0,4);
  $('#alerts').innerHTML = alerts.length ? alerts.map(x=>`<div class="item"><div><b>${x.name}</b><small>${x.qty > 1 ? `${x.qty} normally bought · ` : ''}${x.from} → ${x.to}</small></div><div class="right"><b class="good">save ${money(x.impact)}</b><small>${x.matchQuality}</small></div></div>`).join('') : '<div class="empty">No verified $1+ switches yet.</div>';
  $('#items').innerHTML = household.known_items.map(x=>`<div class="item"><div><b>${x.name}</b><small>${x.size || x.category || ''}</small></div><div class="right"><b>${money(x.paid_price ?? x.last_price ?? x.regular_price)}</b><small>${x.last_store}</small></div></div>`).join('');
}
...
function parseReceiptText(text){
  ...
  parsedCandidates=candidates.slice(0,160);renderCandidates();return{store,total:findTotal(text,store),candidates:parsedCandidates};
}
...
function renderCandidates(){
  ...
  $('#candidates').innerHTML=parsedCandidates.map((c,i)=>`<div class="candidate"><input type="checkbox" ${c.include?'checked':''} data-inc="${i}"><input value="${escapeHtml(c.name)}" data-name="${i}"><input type="number" step="0.01" value="${c.price.toFixed(2)}" data-price="${i}"></div>`).join('');
  $$('[data-inc]').forEach(el=>el.onchange=e=>parsedCandidates[+e.target.dataset.inc].include=e.target.checked);$$('[data-name]').forEach(el=>el.oninput=e=>parsedCandidates[+e.target.dataset.name].name=e.target.value);$$('[data-price]').forEach(el=>el.oninput=e=>parsedCandidates[+e.target.dataset.price].price=Number(e.target.value));
}
function escapeHtml(s){return String(s).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));}

async function runOCR(){
  if(!receiptFile){alert('Take or choose a receipt photo first.');return;}if(!window.Tesseract){alert('OCR library did not load. Paste receipt text instead.');return;}
  $('#ocrStatus').textContent='Starting on-device OCR…';$('#ocrProgress').style.width='2%';
  try{const result=await Tesseract.recognize(receiptFile,'eng',{logger:m=>{if(m.status==='recognizing text')$('#ocrProgress').style.width=Math.round((m.progress||0)*100)+'%';$('#ocrStatus').textContent=m.status+(m.progress!=null?` ${Math.round(m.progress*100)}%`:'');}});$('#receiptText').value=result.data.text;$('#ocrStatus').textContent='OCR complete. Review the text, then confirm parsed lines.';parseReceiptText(result.data.text);}catch(err){$('#ocrStatus').textContent='OCR failed: '+err.message;}
}

function saveReceipt(){
  const confirmed=parsedCandidates.filter(x=>x.include&&x.name&&x.price>0);if(!confirmed.length){alert('No confirmed item lines to save.');return;}
  const text=$('#receiptText').value,selected=$('#storeSelect').value,store=selected==='Auto-detect store'?detectStore(text):selected,date=new Date().toISOString().slice(0,10);const total=findTotal(text,store)??confirmed.reduce((s,x)=>s+x.price,0);
